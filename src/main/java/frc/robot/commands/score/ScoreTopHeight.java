...
public class ScoreTopHeight extends Command {
    ...
    private static class SwitchSideAndGoToTopHeight extends ConditionalCommand {
        public SwitchSideAndGoToTopHeight() {
            super(new PreventElevatorCollision(
                    (Robot.arm.getDesiredSide() == ArmSide.FRONT) ? new ScoreFrontHatch(3) : new ScoreBackHatch(3)),
                    new PreventElevatorCollision((Robot.arm.getDesiredSide() == ArmSide.FRONT) ? new ScoreFrontCargo(3)
                            : new ScoreBackCargo(3)));
    ...
    private static class GoToTopHeight extends ConditionalCommand {
        public GoToTopHeight() {
            super((Robot.arm.getDesiredSide() == ArmSide.FRONT) ? new ScoreFrontHatch(3) : new ScoreBackHatch(3),
                    (Robot.arm.getDesiredSide() == ArmSide.FRONT) ? new ScoreFrontCargo(3) : new ScoreBackCargo(3));
            requires(Robot.intake);
        }

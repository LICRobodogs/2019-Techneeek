...
public class ScoreMiddleHeight extends Command {
    ...
    private static class SwitchSideAndGoToMiddleHeight extends ConditionalCommand {
        public SwitchSideAndGoToMiddleHeight() {
            super(new PreventElevatorCollision(
                    (Robot.arm.getDesiredSide() == ArmSide.FRONT) ? new ScoreFrontHatch(2) : new ScoreBackHatch(2)),
                    new PreventElevatorCollision((Robot.arm.getDesiredSide() == ArmSide.FRONT) ? new ScoreFrontCargo(2)
                            : new ScoreBackCargo(2)));
    ...
    private static class GoToMiddleHeight extends ConditionalCommand {
        public GoToMiddleHeight() {
            super((Robot.arm.getDesiredSide() == ArmSide.FRONT) ? new ScoreFrontHatch(2) : new ScoreBackHatch(2),
                    (Robot.arm.getDesiredSide() == ArmSide.FRONT) ? new ScoreFrontCargo(2) : new ScoreBackCargo(2));
            requires(Robot.intake);
        }
